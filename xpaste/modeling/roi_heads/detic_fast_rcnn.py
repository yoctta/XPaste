...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.xpaste_box_loss=xpaste_box_loss
        self.debug = debug
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'xpaste_box_loss': cfg.MODEL.USE_XPASTE_BOX_LOSS
        })
    ...
    def losses(self, predictions, proposals, \
        ...
        instance_source = (
            cat([p.instance_source for p in proposals if len(p)], dim=0) if len(proposals) else None
        ) if not self.xpaste_box_loss else None
        return {
            "loss_cls": loss_cls, 
            "loss_box_reg": self.box_reg_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes,instance_source,
    ...
    def box_reg_loss(
        self, proposal_boxes, gt_boxes, pred_deltas, gt_classes,instance_source,
        num_classes=-1):
        """
        ...
        if instance_source is not None:
            instance_source=instance_source[fg_inds]
            loss_box_reg=loss_box_reg[instance_source==0]
        return loss_box_reg.sum() / max(loss_box_reg.numel(), 1.0)
